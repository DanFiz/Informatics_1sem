...
def calculate(*args):
    try:
        color0 =expression.get()
        AA=hex(255-int(color0[:2],16))[2:]
        BB=hex(255-int(color0[2:4],16))[2:]
        CC=hex(255-int(color0[4:6],16))[2:]
        color=AA+BB+CC
        ttk.Label(mainframe, text=color0, background=('#'+color0)).grid(column=1, row=2, sticky=W)
        ttk.Label(mainframe,text=color, background=('#'+color)).grid(column=2, row=2, sticky=(W, E))
    except ValueError:
        pass
...
ttk.Label(mainframe, text="Color").grid(column=3, row=1, sticky=W)
#ttk.Label(mainframe, text="Is equivalent to").grid(column=1, row=2, sticky=E)
ttk.Label(mainframe, text="Complementary color").grid(column=3, row=2, sticky=W)
color0=expression.get()

...
class Ball():
    ...
    def move(self):
        ...
        self.set_coords()
        if self.x>=800:
            self.x=800-(self.x-800)/max(abs(self.vx),abs(self.vy),2)
            self.vx=-self.vx/2
        if self.y>=600:
            self.y=600-(self.y-600)/max(abs(self.vy),abs(self.vx),2)
            self.vy=-self.vy/2
        if self.x<=0:
            self.x=-self.x/max(abs(self.vx),abs(self.vy),2)
            self.vx=-self.vx/2
        if self.y<=0:
            self.y=-self.y/max(abs(self.vy),abs(self.vx),2)
            self.vy=-self.vy/2
    ...
    def hittest(self, ob):
        ...
        """
        # TODO
        if (self.r+ob.r)**2>=abs(self.x-ob.x)**2+abs(self.y-ob.y)**2:
            return True
        return False
